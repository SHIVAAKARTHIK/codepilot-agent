...
> **Practice run completed** (`--tui --live`, before the actual recording): processed issues #3 and #4 fully through the real TUI, including the `pr_target` HITL approval prompt resolved by an actual keypress, producing two real PRs — [#5](https://github.com/SHIVAAKARTHIK/codepilot-demo-target/pull/5) and [#6](https://github.com/SHIVAAKARTHIK/codepilot-demo-target/pull/6), both structured exactly per spec (title, labels, commit message, body). Issues #1 and #2 are untouched and reserved for the actual recording.
>
> **Model switched to `gpt-4o-mini`** (`CODEPILOT_MODEL` in `.env`) partway through this session, specifically to get a materially higher tokens-per-minute allowance than `gpt-4o` on a constrained account tier - the 30k TPM limit from the "live-testing fixes" section above was a real recurring interruption. Re-verified `--phase1-check` (write_todos still called correctly) and `--phase4-check` (clean fix, no guardrail violations) against mini before relying on it for the practice run and recording.
